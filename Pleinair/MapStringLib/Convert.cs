/*
 * MapStringLib by Kaplas80
 * https://github.com/Kaplas80/MapStringLib
 */

namespace MapStringLib
{
    using System.Text;

    // Código original: https://source.winehq.org/source/dlls/kernel32/locale.c
    public static class Convert
    {
        ...
        private static char MapToFullWidth(string input, int index)
        {
            ...
            {
                var tableIndex = currentChar - 0x00a2;
            ...
            var n = ComposeKatakana(input, index, out var composedChar);
            if (n > 0)
            {
                return composedChar;
            }
            ...
            {
                var tableIndex = currentChar - 0xffa0;
        ...
        private static int ComposeKatakana(string input, int index, out char composedChar)
        {
            ...
            if (input[index + 1] == 0xff9f)
            {
                if (currentChar >= 0xff8a && currentChar <= 0xff8e)
                {
                    composedChar++;
                    composedChar++;
                }
        ...
        {
            var n = DecomposeKatakana(input, out string output);
        ...
        private static int DecomposeKatakana(char input, out string output)
        {
            ...
            var k = DecomposeKatakanaMap[shift];
            if (k == 0)
            {
            ...
            var sb = new StringBuilder();
            var chr1 = k > 0x50 ? input - (k & 0xf) : DecomposeKatakanaMap[shift - k] | 0xff00;
            var chr2 = k == 2 ? 0xff9f : 0xff9e;
            sb.Append((char)chr1);
            sb.Append((char)chr2);
            output = sb.ToString();
