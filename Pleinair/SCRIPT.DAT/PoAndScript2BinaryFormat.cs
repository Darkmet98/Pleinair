...
namespace Pleinair.SCRIPT.DAT
{
    class PoAndScript2BinaryFormat : IConverter<Po, BinaryFormat>
    {
        ...
        private void GenerateFile()
        {
            ...
            Writer.WriteTimes(0, Result.HeaderSize*4);
        ...
        private byte[] UpdateBlock(byte[] block)
        {
            ...
            for (int i = 0; i < block.Length; i++)
            {
                if(block[i] == 0x32 && block[i - 1] == 0xBE)
                {
                    ...
                    i += Int32.Parse(Source.Entries[PoLine].Reference);
                    PoLine++;
                }
                else
                {
            ...
            Writer.Stream.Write(arrayresult, 0, (int)Writer.Stream.Length);
            return arrayresult;
        }
